...
class Vec{
  ...
  plus(anything) {
      return new Vec (this.x + anything.x, this.y + anything.y);
...
//ugh this took forever to figure out. A lot of debugging and trying to figure out what parameters needed to go inside of minus and plus.
//ultimately realized it could be litterally anything
...
class Group {
  ...
  add(farts){
    if (this.something.includes(!farts));
//if this is false add (push) it to the array
    this.something.push(farts);
  }

  delete(smell){
    this.something = this.something.filter(n => n !== smell);
    //checks the array for the value of n
  }

  has(lol){
  return this.something.includes(lol);
  //checks the array to see if it includes the value
}

  static from(gas) {
    let group = new Group;
    for (let value of gas) {
      group.add(value);
    }
    return group;
  }
